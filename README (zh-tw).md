...
因此 Error vs Angle 圖表上的點，例如 (110, 0.05) 或 (66, 0.07) 就代表這兩處的誤差值是 0.05 與 0.07。圖表的橫軸是由大到小，方便視覺上對應到圓弧上。
...
總之就是有三個未知數，所以需要有三個(不共線的)點來決定該橢圓。如果我們有橢圓上的三個點，就能代入 $x^2+Ay^2+By+C=0$ 得到 $A,B,C$ 的線性方程組，進而求出 $A,B,C$ 並換算成 $a,b,h$。也有可能 $B<0$ 導致求出雙曲線而非橢圓。
