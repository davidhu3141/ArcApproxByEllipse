...
一般來說，當使用者增加誤差容許值，此時由於條件被放寬，應能找到更小的橢圓。但當使用者將容許值設成誇張地過大 (例如大於半徑的 10%)，偏移量的取樣步長也會隨之變大，導致測試的橢圓大部分都被 case 2 過濾掉，因此找不到比圓還小的橢圓。此時可以將 Q sampling steps 調整得更大 (Disable P/R sampling 啟用時最多可設為 3000)。Case 2 的示意圖如下：
