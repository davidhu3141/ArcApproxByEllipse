...
- D3 圖，用來顯示平面座標系的 viewport。 viewport 範圍適當涵蓋圓弧
- D3 圖，用來顯示圓弧各個角度的誤差值
...
目前搜尋的是最小的 a+b 的橢圓。請在進階設定多一個下拉選單，用來選擇搜尋 a 最小的橢圓。預設是選擇搜尋 a 最小的橢圓
...
同 req8，多一個選項用來找 L1+L3 長度最小的橢圓，該長度可參考 req_animation。但預設值不用改。
